( function ( mw ) {
	const oldTitleNewFromImg = mw.Title.newFromImg;

	mw.Title.newFromImg = function ( img ) {
		const title = oldTitleNewFromImg( img );
		let newTitle = null;
		let dummyNSFRTitle = null;
		let src = img.jquery ? img[ 0 ].src : img.src;
		src = decodeURI( src );

		let matches = src.match( /\/([^\s]*\/{1})([^\s]*\/{1})([0-9]+?)\/[a-f0-9]\/[a-f0-9]{2}\/([^\s]+)$/ );
		...
		if ( matches !== null && matches[ 2 ] === 'archive/' ) {
			...
			let fileNameParts = matches[ 4 ].match( /([0-9]{14})!([^\s]+)\/([^\s]+)$/ );
